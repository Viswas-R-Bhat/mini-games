...
export async function submitScore({ usn, game, score, meta }) {
  ...
  // ═══ STEP 1: Update localStorage FIRST (instant, reliable) ═══
  ...
  console.log(`[submitScore] ${game}: localStorage updated — attempts=${newCount}, best=${cached.best}`);

  // ═══ STEP 2: Push to Supabase (async, may fail) ═══
  try {
    const { error: logError } = await supabase
      .from('attempt_logs')
      .insert({ usn: team_name, game, score, meta });

    if (logError) {
      console.error('[submitScore] attempt_logs insert failed:', logError);
    } else {
      console.log(`[submitScore] ${game}: Supabase attempt_logs insert OK`);
    }
  } catch (e) {
    console.error('[submitScore] attempt_logs insert exception:', e);
  }

  // ═══ STEP 3: Upsert best score to game_scores ═══
  try {
    ...
    if (!existing || score > existing.score) {
      const { error: upsertError } = await supabase
        .from('game_scores')
        .upsert(
          { usn: team_name, game, score, meta, updated_at: new Date().toISOString() },
          { onConflict: 'usn,game' }
        );
      if (upsertError) console.error('[submitScore] game_scores upsert failed:', upsertError);
    }
  } catch (e) {
    console.error('[submitScore] game_scores upsert exception:', e);
  }
}
